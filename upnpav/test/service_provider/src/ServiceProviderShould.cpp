...
void ServiceProviderShould::parse_device_description_without_services_no_embedded_services_and_return_values()
{
    ...
    m_providerFactory->serviceDiscoveryBackendDouble->sendNotifyMessage(createServiceDiscoveryReceiveMessage(validNotifyMessage));
    ...
    {
        QVERIFY2(expectedDeviceDescription.icons().at(index).mimeType() == receivedDeviceDescription.icons().at(index).mimeType(),
                 QString{ "The mimetype of icon %1 is not the same" }.arg(index).toUtf8().data());

        QVERIFY2(expectedDeviceDescription.icons().at(index).width() == receivedDeviceDescription.icons().at(index).width(),
                 QString{ "The width of icon %1 is not the same" }.arg(index).toUtf8().data());

        QVERIFY2(expectedDeviceDescription.icons().at(index).height() == receivedDeviceDescription.icons().at(index).height(),
                 QString{ "The height of icon %1 is not the same" }.arg(index).toUtf8().data());

        QVERIFY2(expectedDeviceDescription.icons().at(index).depth() == receivedDeviceDescription.icons().at(index).depth(),
...
void ServiceProviderShould::parse_devices_description_with_services()
{
    auto expectedDeviceDescription = DeviceDescription{
        ...
        QVector<IconDescription>{
            IconDescription{ QString{ "img/png" }, 64, 64, 24, QString{ "http://127.0.0.1/test/img/icon_64_64.png" } },
            IconDescription{ QString{ "img/png" }, 32, 32, 24, QString{ "http://127.0.0.1/test/img/icon_32_32.png" } } },
        QVector<ServiceDescription>{ ServiceDescription{
            QString{ "urn:schemas-upnp-org:service:SERVICE:1" }, QString{ "urn:upnp-org:serviceId:SerivceId5" },
            QString{ "http://127.0.0.1/test/serviceId1.xml" }, QString{ "http://127.0.0.1/test/ctl/serviceId1" },
...
void ServiceProviderShould::ignore_broken_device_description_notify_error()
{
    ...
    ServiceProviderError expectedError{ ServiceProviderError::ErrorCode::XmlNotWellFormed,
                                        QStringLiteral(
                                            "Received XML from http://127.0.0.1:8000/desc.xml is not well formed") };
    ...
    QVERIFY2(expectedError.errorCode() == error.errorCode(), QString{ "Expected: %1 : Actual:%2" }
                                                                 .arg(static_cast<qint32>(expectedError.errorCode()))
                                                                 .arg(static_cast<qint32>(error.errorCode()))
                                                                 .toUtf8()
                                                                 .data());

    QVERIFY2(expectedError.errorDescription() == error.errorDescription(), QString{ "Expected: %1 : Actual:%2" }
...
void ServiceProviderShould::parse_service_control_point_definition()
{
    auto scpdStateVariables = QVector<SCPDStateVariable>{
        ...
        SCPDStateVariable{ false, QString{ "Value2" }, SCPDStateVariable::DataType::Ui2, QString{ "1" },
    ...
    for(; stateVariableIndex < receivedScpd.at(0).serviceStateTable().size(); ++stateVariableIndex)
    {
        ...
        QVERIFY2(
            expectedStateVariable.name() == stateVariable.name(),
            QString{ "Expected: %1 : Actual %2" }.arg(expectedStateVariable.name()).arg(stateVariable.name()).toUtf8().data());

        QVERIFY2(expectedStateVariable.dataType() == stateVariable.dataType(), QString{ "Expected: %1 : Actual %2" }
        ...
        QVERIFY2(
            expectedStateVariable.maximumValue() == stateVariable.maximumValue(),
            QString{ "Expected: %1 : Actual %2" }.arg(expectedStateVariable.name()).arg(stateVariable.name()).toUtf8().data());
    ...
    for(; actionIndex < receivedScpd.at(0).actionList().size(); ++actionIndex)
    {
        ...
        for(; argIndex < actionArgs.size(); ++argIndex)
        {
            ...
            QVERIFY2(expectedActionArgs.at(argIndex).relatedStateVariable() == actionArgs.at(argIndex).relatedStateVariable(),
                     QString{ "Expected %1 : Actual %2" }
