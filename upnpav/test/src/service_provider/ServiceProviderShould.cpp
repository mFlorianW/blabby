...
{
    return QNetworkDatagram{message.toUtf8(),
...
void ServiceProviderShould::send_out_correct_discovery_message()
{
    ...
    QVERIFY2((mSearchDatagram.data() == searchRequest.data()),
             "The search request playlod is not correctly.");
}
...
void ServiceProviderShould::request_device_description_on_received_search_response()
{
    ...
    QVERIFY2(expectedDescriptionUrl == descriptionUrl,
             "The sendout description URL is not expected description URL.");
}
...
void ServiceProviderShould::handle_every_search_response_from_a_device_only_once()
{
    ...
    QVERIFY2(descriptionRequestSend == 1,
             "The MediaServerProvider also handles already known devices.");
}

void ServiceProviderShould::handle_sddp_bye_messages_and_inform_clients_about_the_disconnect()
{
    QSignalSpy signalSpy{m_mediaServerProvider.get(), &ServiceProvider::serviceDisconnected};
...
{
    QSignalSpy signalSpy{m_mediaServerProvider.get(), &ServiceProvider::error};
...
void ServiceProviderShould::ignore_message_with_no_payload_and_notify_clients()
{
    ...
    auto errorObject = signalspy.takeFirst().at(0).value<ServiceProviderError>();
    QVERIFY2(expectedError.errorCode() == errorObject.errorCode(),
             "The expected error code doesn't match");
    QVERIFY2(expectedError.errorDescription() == errorObject.errorDescription(),
...
void ServiceProviderShould::report_error_when_in_message_location_is_missing_and_no_byebye_type()
{
    ...
    QVERIFY2(expectedError.errorCode() == receivedError.errorCode(),
             "The expected error code doesn't match.");
}
...
void ServiceProviderShould::parse_device_description_without_services_no_embedded_services_and_return_values()
{
    auto expectedDeviceDescription = DeviceDescription{
        ...
        QVector<IconDescription>{
            IconDescription{QString{"img/png"}, 64, 64, 24,
                QString{"http://127.0.0.1/test/img/icon_64_64.png"}},
            IconDescription{QString{"img/png"}, 32, 32, 24,
                QString{"http://127.0.0.1/test/img/icon_32_32.png"}}}};
    QSignalSpy signalspy{m_mediaServerProvider.get(), &ServiceProvider::serviceConnected};

    m_providerFactory->serviceDiscoveryBackendDouble->sendNotifyMessage(createServiceDiscoveryReceiveMessage(
                                                                            validNotifyMessage));
    ...
    QVERIFY2(!(expectedDeviceDescription != receivedDeviceDescription),
             "The unequal operator shourld return false");

    QVERIFY2(expectedDeviceDescription.friendlyName()
             == receivedDeviceDescription.friendlyName(),
             "Thre received friendly name is not the same.");

    QVERIFY2(expectedDeviceDescription.manufacturer()
             == receivedDeviceDescription.manufacturer(),
             "Thre received manufacturer name is not the same.");

    QVERIFY2(expectedDeviceDescription.modelName()
             == receivedDeviceDescription.modelName(),
             "Thre received model name is not the same.");

    QVERIFY2(expectedDeviceDescription.udn()
             == receivedDeviceDescription.udn(),
             "Thre received udn name is not the same.");

    QVERIFY2(expectedDeviceDescription.icons().size()
             == receivedDeviceDescription.icons().size(),
    ...
    for(auto index = 0; index < receivedDeviceDescription.icons().size(); ++index)
    {
        ...
        QVERIFY2(expectedDeviceDescription.icons().at(index).url()
                 == receivedDeviceDescription.icons().at(index).url(),
                 QString{"The depth of icon %1 is not the same"}.arg(index).toUtf8().data());
    }
...
void ServiceProviderShould::parse_device_description_with_empty_base_url()
{
    auto expectedPhysicalDeviceDescription = DeviceDescription{
        ...
        QVector<IconDescription>{
            IconDescription{QString{"img/png"}, 64, 64, 24,
                QString{"http://127.0.0.1:8000/img/icon_64_64.png"}},
            IconDescription{QString{"img/png"}, 32, 32, 24,
                QString{"http://127.0.0.1:8000/img/icon_32_32.png"}}}};
    QSignalSpy signalspy{m_mediaServerProvider.get(), &ServiceProvider::serviceConnected};
...
void ServiceProviderShould::parse_devices_description_with_services()
{
    auto expectedDeviceDescription = DeviceDescription{
        ...
        QVector<ServiceDescription>{
            ServiceDescription{QString{"urn:schemas-upnp-org:service:SERVICE:1"},
    ...
    QVERIFY2(expectedDeviceDescription.services().size() == receivedDeviceDescription.services().size(),
             QString{"Expected: %1 : Actual: %2"}
                ...
                .arg(receivedDeviceDescription.services().size()).toUtf8().data());

    for(auto serviceIndex = 0;
        serviceIndex < receivedDeviceDescription.services().size();
        ++serviceIndex)
    {
...
{
    QSignalSpy signalSpy(m_mediaServerProvider.get(), &ServiceProvider::error);
    ServiceProviderError expectedError{ServiceProviderError::ErrorCode::XmlNotWellFormed,
...
void ServiceProviderShould::parse_service_control_point_definition()
{
    auto scpdStateVariables = QVector<SCPDStateVariable>{
            SCPDStateVariable{
                false,
                QString{"Value1"},
                SCPDStateVariable::DataType::Ui1,
                QString{"0"}
            },
            ...
            SCPDStateVariable{
                false,
                QString{"Value16"},
                SCPDStateVariable::DataType::Bool,
                QString{"0"}
            },
    ...
    auto expectedScpd = ServiceControlPointDefinition{QString{"http://127.0.0.1/test/serviceId1.xml"},
        scpdStateVariables,
        scpdActions};
    QSignalSpy signalspy{m_mediaServerProvider.get(), &ServiceProvider::serviceConnected};
    ...
    QVERIFY2(receivedScpd.size() == 1,
             QString{"Expected: 1 : Actual %2"}
                     .arg(receivedScpd.size()).toUtf8().data());
    ...
    QVERIFY2(expectedScpd.serviceStateTable().size() == receivedScpd.at(0).serviceStateTable().size(),
             QString{"Expected: %1 : Actual %2"}
                     ...
                     .arg(receivedScpd.at(0).serviceStateTable().size()).toUtf8().data());
    ...
    QVERIFY2(expectedScpd.actionList().size() == receivedScpd.at(0).actionList().size(),
             QString{"Expected: %1 : Actual %2"}
                     ...
                     .arg(receivedScpd.at(0).actionList().size()).toUtf8().data());
    ...
    for(; actionIndex < receivedScpd.at(0).actionList().size(); ++actionIndex)
    {
        ...
        QVERIFY2(expectedAction.name() == action.name(),
                 QString{"Expected: %1 : Actual %2"}
                         .arg(expectedAction.name())
                         .arg(action.name()).toUtf8().data());
        ...
        QVERIFY2(expectedActionArgs.size() == actionArgs.size(),
                 QString{"Expected: %1 : Actual %2"}
                         .arg(expectedActionArgs.size())
                         .arg(actionArgs.size()).toUtf8().data());
        ...
        for(; argIndex < actionArgs.size(); ++argIndex)
        {
            QVERIFY2(expectedActionArgs.at(argIndex).name() == actionArgs.at(argIndex).name(),
                     QString{"Expected %1 : Actual %2"}
                            ...
                            .arg(actionArgs.at(argIndex).name()).toUtf8().data());
            ...
            QVERIFY2(expectedActionArgs.at(argIndex).direction() == actionArgs.at(argIndex).direction(),
                     QString{"Expected %1 : Actual %2"}
                            ...
                            .arg(actionArgs.at(argIndex).direction()).toUtf8().data());
            ...
            QVERIFY2(expectedActionArgs.at(argIndex).relatedStateVariable() ==
                     ...
                     QString{"Expected %1 : Actual %2"}
                            ...
                            .arg(actionArgs.at(argIndex).relatedStateVariable()).toUtf8().data());
        }
    }

    //Test implemented operators
    QVERIFY2(expectedScpd == receivedScpd.at(0),
             "The equal operator returns wrong value.");

    QVERIFY2(!(expectedScpd != receivedScpd.at(0)),
             "The unequal operator returns wrong value");
}
