...
void GetTransportInfoResponseShould::give_the_current_transport_state_of_a_response()
{
    ...
    auto const expectedTransportState = GetTransportInfoResponse::TransportState::Playing;

    auto const currentTransportState = resp.currentTransportState();
...
void GetTransportInfoResponseShould::give_the_current_transport_status_of_a_response()
{
    auto resp = GetTransportInfoResponse{createGetTransportInfoResponse(),
                                         validAvTranportServiceSCPD(),
                                         createGetTransportInfoAction()};
    auto const expectedTransportStatus = GetTransportInfoResponse::TransportStatus::ErrorOccured;

    auto const currentTransportStatus = resp.currentTransportStatus();
...
void GetTransportInfoResponseShould::give_the_transport_play_speed_of_a_response()
{
    auto resp = GetTransportInfoResponse{createGetTransportInfoResponse(),
                                         validAvTranportServiceSCPD(),
                                         createGetTransportInfoAction()};
    auto const expectedTransportPlaySpeed = QStringLiteral("1/2");

    auto const transportPlaySpeed = resp.transportPlaySpeed();
